...
describe('ActivityChip', () => {
  it('renders the activity label', () => {
    render(<ActivityChip cssClass={getActivityCssClass(ActivityType.WORK)} label="Travail" />)
    expect(screen.getByText('Travail')).toBeInTheDocument()
  })

  it('applies the activity CSS class for color tone', () => {
    const { container } = render(
      <ActivityChip cssClass={getActivityCssClass(ActivityType.WORK)} label="Travail" />
    )
